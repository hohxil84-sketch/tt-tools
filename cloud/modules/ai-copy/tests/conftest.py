...
跨模块导入说明：
    ...
    和 sys.modules 避免模块名冲突（对齐 credits-billing 测试的导入模式）。
"""
...
# 各模块目录路径
_AI_COPY_DIR = os.path.join(_PROJECT_ROOT, "cloud", "modules", "ai-copy")
...
from service import (
    ...
    consume_credits as cb_consume_credits,
)  # noqa: E402

# 保留 credits-billing 的 service 引用（用于手动调用扣费/赠送）
_cb_consume_credits = cb_consume_credits
...
# ============================================================
# 步骤 4: 设置 ai-copy 模块目录到 sys.path（用于 router 导入）
# ============================================================
...
async def free_user(db_session: AsyncSession, seed_plans_fixture):
    ...
    免费套餐的 ai_copy_cloud 功能未启用，应返回权限错误。
    """
    user = User(
