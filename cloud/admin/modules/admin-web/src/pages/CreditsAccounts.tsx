...
interface Acc { id: string; user_id: string; user_account: string | null; plan_code: string; balance: number; monthly_grant: number; status: string; period_start: string | null; period_end: string | null; updated_at: string; user_display_name?: string | null; created_at?: string; }
...
export default function CreditsAccounts() {
  ...
  const [sf, setSf] = useState(''); const [pc, setPc] = useState('');
  const [pg, setPg] = useState(0); const [err, setErr] = useState('');
  const [detail, setDetail] = useState<Acc | null>(null);

  const load = useCallback(async () => {
    setErr('');
    try { setD(await apiRequest<List>('/admin/credits/accounts', { params: { limit: PAGE, offset: pg * PAGE, status: sf || undefined, plan_code: pc || undefined } })); }
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', gap: 10, marginBottom: 20 }}>
        ...
        <input placeholder="套餐编码" value={pc} onChange={e => { setPc(e.target.value); setPg(0); }} style={inpS} />
