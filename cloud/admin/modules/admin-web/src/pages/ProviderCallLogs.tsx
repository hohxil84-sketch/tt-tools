...
export default function ProviderCallLogs() {
  ...
  const load = useCallback(async () => {
    setErr('');
    try { setD(await apiRequest<List>('/admin/provider-call-logs', { params: { limit: PAGE, offset: pg * PAGE, user_id: uid || undefined, feature: feat || undefined, provider: prov || undefined, status: sf || undefined } })); }
  ...
  return (
    <div>
      ...
      <Card><Tbl heads={['用户', '功能', 'Provider', '模型', '状态', 'Token', '成本', '额度', '延迟', '时间', '']}>
        {d?.items.map(l => (
          <tr key={l.id}>
            ...
            <td style={{ fontSize: 12 }}>${l.estimated_cost?.toFixed(4)}</td>
