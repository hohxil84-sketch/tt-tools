...
- Bug 修复也必须走分支流程，不得直接在 `dev/full-product` 或 `main` 上修改。
- 修复前必须确认 bug 归属模块，并从最新 `dev/full-product` 创建 `fix/<模块名>-<问题简述>` 分支。
- 修复分支只能包含该 bug 的最小必要改动，不得混入无关重构、新功能或其他模块改动。
- 涉及接口字段、OpenAPI、DTO、数据库结构或模块调用关系的 bug，必须先更新对应权威规格文档，再修改实现代码。
- 修复前必须尽量复现问题并定位根因；修复后必须重新运行失败用例、受影响模块完整测试，涉及接口时还要运行相关契约或客户端测试。
- 修复完成并自测通过后，必须停止并等待用户测试；只有用户明确确认测试通过后，才能提交和推送修复分支。
- 修复完成后必须更新对应模块 `PROGRESS.md` 的 Bug 记录或测试记录，写明 bug 现象、根因、修复点、测试命令、测试结果和分支名；提交哈希必须在用户确认通过并完成提交后补充。
- Bug 修复提交信息必须使用中文，并使用 `fix(<模块名>): <修复说明>` 格式。
- 修复分支必须在用户测试通过后推送到 origin；是否合并到 `dev/full-product` 由用户确认。
- 影响当前联调、当前可运行版本、主流程或已合并模块稳定性的 bug，用户测试通过并确认后应合并到 `dev/full-product`。
- 实验性修复、未验收修复或范围不清的修复，不得提交或推送，必须等待用户确认测试通过。
...
- Bug 修复完成并自测通过后，必须等待用户测试确认通过，确认前不得提交、推送或合并。
